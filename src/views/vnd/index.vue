...
const getQrcode = () => {
  ...
  qrCodeUrl.value = apiBase + '/api/payments-qrcode/' + id.value + '?rand=' + rand
}
...
const init = async () => {
  ...
  if (!apiBase) {
    ...
    configError.value = 'VITE_API_URL is not set. Configure it in Cloudflare Pages environment variables and redeploy.'
    load.value = false
    return
  }

  try {
    const response = await axios.get(apiBase + '/api/payments/' + id.value)
    const data = response.data?.data
    if (!data || typeof data !== 'object') {
      console.error('Invalid payment response', response.data)
      configError.value = 'Invalid payment response from API.'
      load.value = false
      return
    }
    console.log(data)
    ...
    console.log(error)
    configError.value = 'Failed to load payment. Check API domain / CORS / network.'
    load.value = false
    return
  }
...
const goPay = async () => {
  ...
  await axios
    .put(apiBase + '/api/payments-selectBank', { bank_name: order.value.bank_name, trade_no: id.value })
...
<template>
  ...
  <div v-if="!load" class="flex flex-center" style="max-width:924px;margin:auto">
    ...
    <div v-if="configError" class="pay-close-desc" style="text-align: center;margin: 40px 20px">
